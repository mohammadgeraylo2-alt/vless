...
async def _grant_free_config(bot, user_id, gb, days, title, fail_note):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=ADMIN_ID,
            text=f"❌ ساخت کانفیگ جایزه برای {user_id} شکست خورد: {e}"
        )
        return False
    ...
    async with _db_lock:
        ...
        if first_time_verified and record["referred_by"] and not record["referral_credited"]:
            ...
            if str(inviter_id) != str(user.id):
                ...
                inviter["referred_usernames"].append(
                    record["username"] or "کاربر ناشناس"
                )
                ...
                try:
                    me = await bot.get_me()
                    progress_text = _build_referral_text(
                        me.username,
                        inviter_id,
                        progress_snapshot
                    )
                    ...
                    logger.warning(
                        "نشد به دعوت‌کننده %s پیام بدیم (شاید بلاک کرده)",
                        inviter_id
                    )
                ...
                if reward_ready:
                    await _grant_free_config(
                        ...
                        fail_note=(
                            "⚠️ تبریک، به هدف رسیدی! ولی تو ساخت کانفیگ یه خطای موقت پیش اومد. "
        ...
        welcome_bonus_due = (
            first_time_verified
            and record["referred_by"]
            and not record["welcome_bonus_claimed"]
        )
...
async def _verify_and_greet(bot, chat_id, user, edit_message=None):
    ...
    if welcome_bonus_due:
        ...
        await _grant_free_config(
            ...
            fail_note=(
                f"⚠️ خوش اومدی! ولی تو ساخت کانفیگ {gb_label} گیگی یه خطای موقت پیش اومد. "
                "لطفاً /start رو دوباره بزن."
            ),
        )

    await _send_referral_panel(
        bot,
        chat_id,
        user.id,
        record,
...
async def check_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await _is_member_of_all_channels(context.bot, user.id):
        ...
        await query.answer(
            "❌ هنوز عضو همه‌ی کانال‌ها نشدی. اول جوین کن، بعد دوباره بزن.",
            show_alert=True
        )
...
async def addchannel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "فرمت درست: /addchannel @channel_username"
        )
        return
...
async def removechannel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "فرمت درست: /removechannel @channel_username (یا شماره‌ش از /channels)"
        )
        return
    ...
    async with _channels_lock:
        ...
        if target.isdigit():
            ...
            if idx < 0 or idx >= len(channels):
                await update.message.reply_text(
                    "شماره نامعتبره. /channels رو بزن تا لیست رو ببینی."
                )
                return
    ...
    await update.message.reply_text(
        f"🗑 {removed} از لیست جوین اجباری حذف شد."
    )
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ranked = sorted(
        db.items(),
        key=lambda item: item[1].get("total_referrals", 0),
        reverse=True
    )
    ...
    ranked = [
        ...
        if rec.get("total_referrals", 0) > 0
    ]

    if not ranked:
        await update.message.reply_text(
            "هنوز هیچ‌کس کسی رو دعوت نکرده."
        )
        return
    ...
    for i, (uid, rec) in enumerate(ranked[:30]):
        ...
        lines.append(
            f"{rank_icon} {name} — {rec['total_referrals']} نفر"
        )
...
async def ask_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "چند روز اعتبار داشته باشه؟ (فقط عدد)"
    )
    return ASK_DAYS
...
async def revoke_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["revoke_map"] = {
        str(i + 1): c["uuid"] for i, c in enumerate(clients)
    }
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(
            refresh_referral_callback,
            pattern="^refresh_referral$"
        )
    )
